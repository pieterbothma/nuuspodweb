...
export function GidsKaarte() {
  ...
  return (
    <section id="gidse" aria-labelledby="gidse-kop" className="scroll-mt-24">
      <h2 id="gidse-kop" className="font-sans text-xs font-bold tracking-[0.22em] text-ink uppercase">
        Hoe om te stem
      </h2>
      <ul className="border-rand bg-rand mt-4 grid gap-px border sm:grid-cols-2">
        {gidse.map((g) => (
          <li key={g.slug} className="bg-grond">
            <Link href={`/gids/${g.slug}`} className="group block h-full p-5 focus-visible:outline-2 focus-visible:outline-rooi">
              <span className="text-ink group-hover:text-rooi block font-display text-xl text-balance">{g.titel}</span>
