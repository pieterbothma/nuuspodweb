  ...
  return (
    <section id="program" aria-labelledby="program-kop" className="scroll-mt-24">
      ...
      <div className="border-rand bg-paneel mt-6 overflow-hidden border">
        ...
        <p className="text-ink px-5 py-4 font-sans text-sm">{nuutste.titel}</p>
      </div>
      {vorige.length > 0 && (
        <ul className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3">
          {vorige.map((e) => (
            <li key={e.video_id}>
              <a href={`https://www.youtube.com/watch?v=${e.video_id}`} target="_blank" rel="noopener noreferrer" className="group block focus-visible:outline-2 focus-visible:outline-rooi">
                <Image
                ...
                />
                <span className="text-grys group-hover:text-rooi mt-2 block font-sans text-sm">{e.titel}</span>
              </a>
            </li>
          ))}
        </ul>
