-- Verkiesing 2026, Fase 2b, Task 1 — herstelrondte 1, punt 2: 'n alias gee EEN ry.
-- Toegepas op projek xxysgvanarnirxoxrbkj met die Supabase MCP apply_migration-hulpmiddel.
--
-- Die taakbrief het gesê die etiket van 'n alias-treffer is `plekke.naam`, so "Kaapstad"
-- het 126 groepe opgelewer waarvan `limit 20` toe 20 los Kaapse sub-pleknaam gewys het
-- ("Mahikeng" net so). Dit weerspreek Piet se eie besluit in die globale beperkings —
-- "search groups places per main place/municipality rather than listing sub places" —
-- en die koördineerder het beslis dat die beperking wen.
--
-- Nuwe reël: 'n alias-treffer gee een ry per (alias, munisipaliteit), geëtiketteer met
-- die alias in sy kanonieke vorm soos dit in `plek_aliasse.alias` staan ("Kaapstad"),
-- met die wyke van AL sy sub-plekke in `wyk_ids`/`wyk_nrs` saamgevoeg. Naam- en
-- hoofplek-treffers bly presies soos hulle was.
--
-- Struktuur: waar die etiket voorheen per sp_kode bepaal is (een etiket per plek), is dit
-- nou 'n (etiket, sp_kode)-paar, want dieselfde plek kan onder sy alias EN onder sy eie
-- naam opduik. Die bestaande `group by etiket, muni_kode` doen dan die werk, en waar 'n
-- alias toevallig dieselfde etiket as die pleknaam het (bv. "Bloemfontein") smelt die twee
-- in een ry saam i.p.v. te dupliseer. Die "verkies die sub-pleknaam as albei tref"-reël
-- geld nog steeds tussen 'n naam- en 'n hoofplek-treffer.
